...
% Input:
%
%  X : a 3xN array containing the point coordinates in 3D world coordinates (meters)
%
...
assert(size(X,1)==3,'X should be a 3xN array');
assert(size(cam.f,1)==1,'cam.f should be a scalar');
assert(size(cam.c,1)==2,'cam.c should be a 2x1 vector');
assert(all(size(cam.R)==[3 3]),'cam.R should be a 3x3 matrix');
assert(all(size(cam.t)==[3 1]),'cam.t should be a 3x1 matrix');
